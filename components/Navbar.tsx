...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className={`bg-white/50 backdrop-blur-sm rounded-2xl ${isScrolled ? '' : 'border border-gray-100/50'} px-2 py-2 md:px-6 md:py-3 flex items-center justify-between transition-all relative`}>
            ...
            {/* Logo */}
            <Link to="/" className="flex items-center gap-2 group cursor-pointer z-50" onClick={() => window.scrollTo(0,0)}>
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <Link to="/case-studies" className="text-[14px] font-medium text-gray-600 hover:text-navy-900 transition-colors py-4" onMouseEnter={() => setActiveDropdown(null)}>
                Case Studies
              </Link>
              
              <Link to="/about" className="text-[14px] font-medium text-gray-600 hover:text-navy-900 transition-colors py-4" onMouseEnter={() => setActiveDropdown(null)}>
                About
              </Link>
            </div>
      ...
      <div 
        className={`fixed inset-0 bg-navy-900/40 backdrop-blur-md z-[50] transition-opacity duration-500 md:hidden ${
      ...
      {/* Mobile Menu Drawer */}
      <div 
        className={`fixed inset-y-0 right-0 w-full sm:w-[320px] bg-white z-[60] shadow-2xl transition-transform duration-500 cubic-bezier(0.32, 0.72, 0, 1) md:hidden flex flex-col ${
      ...
      >
        <div className="flex flex-col h-full relative">
          ...
          <div className="flex-1 overflow-y-auto py-6 px-6 space-y-2">
             ...
             <Link to="/case-studies" onClick={() => setIsMobileMenuOpen(false)} className="block py-2 text-lg font-bold text-navy-900">Case Studies</Link>
             <Link to="/blog" onClick={() => setIsMobileMenuOpen(false)} className="block py-2 text-lg font-bold text-navy-900">Blog</Link>
             <Link to="/about" onClick={() => setIsMobileMenuOpen(false)} className="block py-2 text-lg font-bold text-navy-900">About</Link>
             <Link to="/careers" onClick={() => setIsMobileMenuOpen(false)} className="block py-2 text-lg font-bold text-navy-900 flex items-center justify-between">
                Careers 
                <span className="px-2 py-0.5 bg-brand/10 text-brand text-xs font-bold rounded-full border border-brand/20">HIRING</span>
             </Link>
             <Link to="/contact" onClick={() => setIsMobileMenuOpen(false)} className="block py-2 text-lg font-bold text-navy-900">Contact</Link>
          </div>
